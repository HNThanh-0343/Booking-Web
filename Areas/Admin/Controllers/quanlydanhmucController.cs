...
namespace WEBSITE_TRAVELBOOKING.Areas.Admin.Controllers
{
    ...
    public class quanlydanhmucController : Controller
    {
        ...
        #region Danh mục tính năng xe
        public IActionResult IndexTinhNangXe()
        {
            return View();
        }
        public IActionResult childIndexTinhNangXe(int? page, string searchValue)
        {
            try
            {
                var getAccount = Account.GetAccount();
                ...
                var listFeature = _unitOfWork.Repository<CatCarFeature>().GetAll();
                if (!string.IsNullOrEmpty(searchValue))
                {
                    listFeature = listFeature.Where(h => (h.Name ?? "").ToUpper().Contains(searchValue.ToUpper())).ToList();
                }
                #region ViewBag
                ViewBag.searchValue = searchValue;
                #endregion
                ...
                page = page < 1 ? 1 : page;
                var pageSize = 10;
                var pageListView = listFeature.ToPagedList(page ?? 1, pageSize);
                if (pageListView != null)
                {
                    return PartialView("childIndexTinhNangXe", pageListView);
                }
                return PartialView("childIndexTinhNangXe");
            }
            catch (Exception)
            {
                throw;
        ...
        public IActionResult themMoiTinhNangXe()
        {
            try
            {
                CatCarFeature catCarFeature = new CatCarFeature();
                return PartialView("themMoiTinhNangXe", catCarFeature);
            }
            catch (Exception)
            {
        ...
        [HttpPost]
        public IActionResult themMoiTinhNangXe(CatCarFeature catCarFeature)
        {
            try
            {
                var getAccount = Account.GetAccount();
                ...
                _unitOfWork.Repository<CatCarFeature>().Insert(catCarFeature);
                return PartialView(catCarFeature);
            }
            catch (Exception)
            {
        ...
        public IActionResult chinhSuaTinhNangXe(int id)
        {
            try
            {
                var catCarFeature = _unitOfWork.Repository<CatCarFeature>().GetById(id);
                if (catCarFeature == null)
                {
                    return BadRequest();
                }
                return PartialView("chinhSuaTinhNangXe", catCarFeature);
            }
            catch (Exception)
            {
        ...
        [HttpPost]
        public IActionResult chinhSuaTinhNangXe(CatCarFeature catCarFeature)
        {
            try
            {
                var getCarFeatures = _unitOfWork.Repository<CatCarFeature>().GetById(catCarFeature.Id);
                ...
                getCarFeatures.Name = catCarFeature.Name;
                getCarFeatures.Icon = catCarFeature.Icon;
                getCarFeatures.Status = catCarFeature.Status;
                _unitOfWork.Repository<CatCarFeature>().Update(getCarFeatures);
                return PartialView(getCarFeatures);
            }
            catch (Exception)
            {
                throw;
        ...
        public IActionResult chiTietTinhNangXe(int id)
        {
            try
            {
                var catCarFeature = _unitOfWork.Repository<CatCarFeature>().GetById(id);
        ...
        public IActionResult xoaTinhNangXe(int id)
        {
            try
            {
                var catCarFeature = _unitOfWork.Repository<CatCarFeature>().GetById(id);
                if (catCarFeature == null)
                {
                    return Json(new { result = false, message = "Không tìm thấy tính năng" });
                }
                _unitOfWork.Repository<CatCarFeature>().Delete(catCarFeature);
                return Json(new { result = true, message = "Xóa thành công" });
            }
            catch (Exception)
            {
