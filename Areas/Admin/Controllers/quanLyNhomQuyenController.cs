    ...
    public class quanLyNhomQuyenController : Controller
    {
        ...
        public IActionResult childIndex(int? page, string searchValue)
        {
            ...
            if (getAccout.IdRole == 1)
            {
                ...
                var getRoleAll = _unitOfWork.Repository<SysRole>().GetAll(filter: (m => m.IdUserPrent == null)).ToList();
                if (!string.IsNullOrEmpty(searchValue))
                {
                    getRoleAll = getRoleAll.Where(m => m.Name.ToLower().Contains(searchValue.ToLower())).ToList();
                }
                #region Page
                page = page == null ? 1 : page;
                page = page < 1 ? 1 : page;
                var pageSizeAll = 10;
                var pageListViewAll = getRoleAll.ToPagedList(page ?? 1, pageSizeAll);
                #endregion
                #region ViewBag
                ViewBag.Partner = true;
                ViewBag.ListPartner = _unitOfWork.Repository<SysUser>().GetAll(m => m.IdRole == 3);
                #endregion
        ...
        public IActionResult phanquyen(int id)
        {
            try
            {
                ...
                if (getAccount.IdRole == 1)// quyền admin
                {
                    ...
                    var getModule = _unitOfWork.Repository<SysModule>().GetAll(filter: (m => m.Status == true));
                    ...
                    var tree = BuildRoleTree(mapRuleModule);
        ...
        public List<ModelsAndRole> BuildRoleTree(List<ModelsAndRole> flatList, string PrentId = null)
        {
            // Tìm các node có PrentId khớp
            var nodes = flatList
                .Where(m => m.PrentId == PrentId)
                .OrderBy(m => m.Order ?? 0) //  Sắp xếp node hiện tại theo Order
                .ToList();

            foreach (var node in nodes)
            {
                // Gán Children bằng kết quả đệ quy
                node.Children = BuildRoleTree(flatList, node.IdModule.ToString());

                //  Sắp xếp Children (nếu có)
                node.Children = node.Children.OrderBy(c => c.Order ?? 0).ToList();
            }
